...
struct clan* inter_clan_fromsql(int clan_id){
	...
	clan->id = clan_id;
	Sql_GetData(sql_handle,  0, &data, &len); memcpy(clan->name, data, min_v(len, NAME_LENGTH));
	Sql_GetData(sql_handle,  1, &data, &len); memcpy(clan->master, data, min_v(len, NAME_LENGTH));
	Sql_GetData(sql_handle,  2, &data, &len); memcpy(clan->map, data, min_v(len, MAP_NAME_LENGTH_EXT));
